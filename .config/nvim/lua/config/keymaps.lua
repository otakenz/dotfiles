...
MAP("x", "<C-p>", ":<C-u>execute \"'<,'>move '<-\" . (v:count1 + 1)<CR>gv=gv", { desc = "Move Up" })
MAP("x", "<C-n>", ":<C-u>execute \"'<,'>move '>+\" . v:count1<CR>gv=gv", { desc = "Move Down" })
...
MAP("x", "gt", "c<C-r>=system('tcc', getreg('\"'))[:-2]<CR>", { desc = "Titleize Text" })
...
MAP("n", "<leader>al", ":diffget LO<CR>", { desc = "Accept LOCAL" })
MAP("n", "<leader>ab", ":diffget BA<CR>", { desc = "Accept BASE" })
MAP("n", "<leader>ar", ":diffget RE<CR>", { desc = "Accept REMOTE" })
